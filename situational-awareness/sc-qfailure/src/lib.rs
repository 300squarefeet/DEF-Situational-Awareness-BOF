...
//
//! QueryServiceConfig2A SERVICE_CONFIG_FAILURE_ACTIONS — failure recovery actions.
//! Args: <servicename>
#![no_std]
...
// SC_ACTION_TYPE values
const SC_ACTION_NONE: u32    = 0;
const SC_ACTION_RESTART: u32 = 1;
const SC_ACTION_REBOOT: u32  = 2;
const SC_ACTION_RUN_COMMAND: u32 = 3;
...
fn run(parser: &mut rustbof::data::DataParser) -> Result<(), &'static str> {
    ...
    if svc_s.is_empty() {
        return Err("usage: sc-qfailure <servicename>");
    }
    ...
    // Two-pass QueryServiceConfig2A for SERVICE_CONFIG_FAILURE_ACTIONS
    ...
    let buf_size = if bytes_needed == 0 { 1024 } else { bytes_needed };
    ...
    // SERVICE_FAILURE_ACTIONSA layout (x64):
    // u32  dwResetPeriod   @ offset 0
    // pad  4               @ offset 4
    // *i8  lpRebootMsg     @ offset 8
    // *i8  lpCommand       @ offset 16
    // u32  cActions        @ offset 24
    // pad  4               @ offset 28
    // *SC_ACTION lpsaActions @ offset 32
    //
    // SC_ACTION = { u32 Type, u32 Delay } = 8 bytes each
    let reset_period = unsafe { core::ptr::read_unaligned(buf.as_ptr() as *const u32) };
    let reboot_ptr   = unsafe { core::ptr::read_unaligned(buf.as_ptr().add(8)  as *const *const i8) };
    let cmd_ptr      = unsafe { core::ptr::read_unaligned(buf.as_ptr().add(16) as *const *const i8) };
    let c_actions    = unsafe { core::ptr::read_unaligned(buf.as_ptr().add(24) as *const u32) };
    let actions_ptr  = unsafe { core::ptr::read_unaligned(buf.as_ptr().add(32) as *const *const u8) };

    println!("Service      : {}", svc_s.as_str());
    println!("ResetPeriod  : {} seconds", reset_period);

    if !reboot_ptr.is_null() {
        let msg = ptr_to_cstr(reboot_ptr as *const u8, 512);
        println!("RebootMsg    : {}", msg);
    }
    if !cmd_ptr.is_null() {
        let cmd = ptr_to_cstr(cmd_ptr as *const u8, 512);
        println!("Command      : {}", cmd);
    }

    println!("Actions      : {} configured", c_actions);
    if !actions_ptr.is_null() {
        for i in 0..c_actions as usize {
            let action_base = unsafe { actions_ptr.add(i * 8) };
            let action_type  = unsafe { core::ptr::read_unaligned(action_base as *const u32) };
            let action_delay = unsafe { core::ptr::read_unaligned(action_base.add(4) as *const u32) };
            ...
            println!("  [{}] Type={} Delay={}ms", i + 1, type_str, action_delay);
        }
    }
