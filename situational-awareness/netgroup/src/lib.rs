...
//
//! Domain groups via NetGroupEnum (level 0).
//! Args: [domain]
#![no_std]
...
const TECHNIQUES: &[Technique] = &[
    Technique { id: "T1069.002", name: "Permission Groups Discovery: Domain Groups", tactic: "Discovery" },
];
...
// GROUP_INFO_0: just a *u16 name
#[repr(C)]
struct GroupInfo0 { grpi0_name: *const u16 }
...
fn run(parser: &mut rustbof::data::DataParser) -> Result<(), &'static str> {
    ...
    loop {
        ...
        for i in 0..(entries_read as usize) {
            let entry = unsafe { &*(buf.add(i * core::mem::size_of::<GroupInfo0>()) as *const GroupInfo0) };
            let name = wide_to_str(entry.grpi0_name, 64);
            println!("  {}", name);
            total_printed += 1;
        }
