...
fn run(parser: &mut rustbof::data::DataParser) -> Result<(), &'static str> {
    ...
    if path_s.is_empty() { return Err("usage: md5 <filepath>"); }
    ...
    let ok2 = unsafe { crypt_create_hash(h_prov, 0x8003u32, 0, 0, &mut h_hash) }.unwrap_or(0);
    if ok2 == 0 {
        unsafe {
    ...
    // CryptGetHashParam — HP_HASHVAL = 0x0002, MD5 = 16 bytes
    ...
    println!("MD5({}): {}", path_s.as_str(), hex_str(&hash_buf[..hash_len as usize]));
    Ok(())
}
