...
// vtable indices:
// ITaskService: [0=QI, 1=AddRef, 2=Release, 3=Connect, 4=GetFolder, ...]
// ITaskFolder:  [0=QI, 1=AddRef, 2=Release, 3=get_Name, 4=get_Path, 5=GetFolder, 6=GetFolders, 7=GetTask, 8=GetTasks, ...]
// IRegisteredTaskCollection: [0=QI, 1=AddRef, 2=Release, 3=get_Count, 4=get_Item, ...]
// IRegisteredTask: [0=QI, 1=AddRef, 2=Release, 3=get_Name, 4=get_Path, 5=get_Definition, 6=get_Enabled, 7=get_LastRunTime, ...]
...
fn run() -> Result<(), &'static str> {
    ...
    // ITaskService::Connect(serverName=NULL, user=NULL, domain=NULL, password=NULL)
    // vtbl[3] = Connect
    ...
    // ITaskFolder::GetTasks = vtbl[8] (flags=0)
    ...
    println!("SCHEDULED TASKS ({}):", count);
    println!("{}", "--------------------------------------------");

    // get_Item = vtbl[4] (index: VARIANT)
    ...
    for i in 1..=count {
        ...
        // get_Name = vtbl[3], get_Path = vtbl[4]
        ...
        if hr_name == S_OK && !name_bstr.is_null() {
            let name = wide_to_str(name_bstr, 128);
            println!("  {}", name);
